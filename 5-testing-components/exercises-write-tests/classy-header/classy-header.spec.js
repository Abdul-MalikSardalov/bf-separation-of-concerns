...
describe('classyHeader: renders DOM headers of different levels and classes', () => {
    ...
    describe('an H1 with text: "good bye", and two classes', () => {
        const actual = classyHeader(1, 'good bye', ['fancy', 'hover']);
        it('has tagName "H1"', () => {
        ...
        it('have class: "fancy"', () => {
            const clasesInclude = actual.classList.contains('fancy');
            expect(clasesInclude).toEqual(true);
        });
        it('have class: "hover-right"', () => {
            const clasesInclude = actual.classList.contains('hover');
            expect(clasesInclude).toEqual(true);
        });
        it('has childElementCount: 0', () => {
            expect(actual.childElementCount).toEqual(0);
